...
# l= []
result=[]
x_cols = [x for x in df.columns if x != 'acceptability']
for var1 in x_cols:
    u = df[var1].unique()
...
#     print (var1)
    ...
    for var2 in x_cols:
        ...
        l= []
        y = np.mean(df[var2])
        for j,k in zip(df[var1],df[var2]):
            s = s + ((j-x)*(k-y))
            sigma_x= sigma_x+(j-x)**2
            sigma_y= sigma_y+(k-y)**2
...
#     print(result)
#     print('\n')
# result.reshape(len(x_cols),len(x_cols))
# np.array(result).reshape(len(x_cols),len(x_cols) )
# print(result)
